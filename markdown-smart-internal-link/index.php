...
function fn_markdown_replace_link($content, $lot) {
    ...
    return preg_replace_callback('#(?:\[(.*?)\])?\[link:((?:\.{2}/)*)([a-z\d/-]*?)([?&\#].*?)?\]#', function($m) use(&$links, $language, $lot, $url) {
        ...
        $title_text = Page::apart(file_get_contents($f));
        $title_text = isset($title_text['title']) ? $title_text['title'] : $t;
...
Hook::set(['page.content', 'page.description'], 'fn_markdown_replace_link', 1.9);